...
import { FiUser, FiMail, FiPhone, FiUserCheck, FiGift } from 'react-icons/fi';
...
export default function ProfilePage() {
  ...
  // Fetch latest membership status & points
  useEffect(() => {
    if (!user) return;
    ...
    fetch('http://localhost:5003/api/v1/auth/membership', {
      ...
      .then(json => {
        if (json.success && json.data) {
          setMembershipStatus(json.data.status);
          setMembershipPoints(
            typeof json.data.points === 'number' ? json.data.points : null
          );
        }
      })
          ...
          <dl>
            ...
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 flex items-center">
                ...
                {/* Button group pushed to the right */}
